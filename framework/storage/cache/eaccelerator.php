...
class Eaccelerator extends Cache {
    ...
    public function set($key, $val, $life_time = null) {
        ...
        if (DEBUG) \Lysine\logger('cache')->debug('Eaccelerator set key '. is_array($key) ? implode(',', $key) : $key .' with life_time '. $life_time);
    ...
    public function get($key) {
        if (DEBUG) \Lysine\logger('cache')->debug('Eaccelerator get key '. is_array($key) ? implode(',', $key) : $key);
    ...
    public function delete($key) {
        if (DEBUG) \Lysine\logger('cache')->debug('Eaccelerator delete key '. is_array($key) ? implode(',', $key) : $key);
